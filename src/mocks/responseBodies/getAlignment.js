export const GET_ALIGNMENT_RESPONSE = {
  overallSimilarityScore: 70,
  topMatchPercent: 90,
  topMatchValue: 'benevolence',
  userAName: 'Steve',
  userBName: 'Billy',
  valueAlignment: [
    {
      description:
        'Being reliable and devoted to the needs of those around you gives you great satisfaction; you likely do a great deal to keep your close relationships thriving.',
      score: 90,
      name: 'benevolence',
      id: 'benevolence',
    },
    {
      description:
        'The sense of security that comes with feeling like you belong highly motivates you. Comfort for you is knowing the people and things you cherish are taken care of.',
      score: 85,
      name: 'security',
      id: 'security',
    },
    {
      description:
        'At its essence, Self-Direction is a need for control, mastery, and independence - so it\u2019s no surprise you might relish in choosing your own goals - and honing till they are just right. Likely curious, intelligent and a lover of freedom - you prefer to live by your own rules.',
      score: 80,
      name: 'self direction',
      id: 'self_direction',
    },
    {
      description:
        "Power is invaluable. After all, it likely enables you to control the people and situations around you. When you're on top making the decision, you feel most yourself.",
      score: 75,
      name: 'power',
      id: 'power',
    },
    {
      description:
        'Broadminded and selfless, you likely focus on bolstering social justice and equality so that the world is more fair and peaceful for all.',
      score: 70,
      name: 'universalism',
      id: 'universalism',
    },
    {
      description:
        'Likely a perfectionist at heart, you strive to be the best you can be. And if you get recognition and approval in the process, that really is the cherry on top.',
      score: 65,
      name: 'achievement',
      id: 'achievement',
    },
    {
      description:
        'The idea of upsetting others or disrupting the status quo likely fills you with dread, so you work hard to bring self-discipline, responsibility and politeness to the table.',
      score: 60,
      name: 'conformity',
      id: 'conformity',
    },
    {
      description:
        'As a person, you care deeply for humility and accepting your place in the world. Most likely you are a religious person and find following your faith to be very important.',
      score: 55,
      name: 'tradition',
      id: 'tradition',
    },
    {
      description:
        'Whether through exploring the world or indulging in your favorite food, you likely love instant gratification and value decisions that reward your senses.',
      score: 50,
      name: 'hedonism',
      id: 'hedonism',
    },
    {
      description:
        'The world is a big place and you see the benefits of craving new experiences. On top of a need for variety and stimulation, you may also be a bit daring compared to most. You are likely a go-getter change-maker at heart.',
      score: 45,
      name: 'stimulation',
      id: 'stimulation',
    },
  ],
};
