...
describe('Registration Page', () => {
  ...
  it('User gets an error if the name is invalid', async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <Register />;
      </QueryClientProvider>
    ...
    const fullName = await screen.getByLabelText(/full name/i);
    const confirmPassword = await screen.getByLabelText(/confirm password/i);
    await wait(() => userEvent.type(fullName, 'T'));
    await wait(() => userEvent.type(confirmPassword, 'P'));
    const errorMessage = await waitForElement(() =>
      screen.getByText(/must be at least 2 characters/i)
    );
  });

  it('User gets an error if the email is invalid', async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <Register />;
      </QueryClientProvider>
  ...
  it('User gets an error if the password is invalid', async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <Register />;
      </QueryClientProvider>
  ...
  it('User gets an error if the passwords do not match', async () => {
    render(
      <QueryClientProvider client={queryClient}>
        <Register />;
      </QueryClientProvider>
